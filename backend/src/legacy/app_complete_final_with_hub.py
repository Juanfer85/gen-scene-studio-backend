...
log = setup_logging()
app = FastAPI(title="Gen Scene Studio Backend", version="1.0.0-HUB-FIXED")
...
def process_job_background(job_id: str, job_type: str):
    """Process a job in the background - this runs synchronously"""
    try:
...
def list_jobs_hub(_k=Depends(require_api_key)):
    """Hub Jobs endpoint - lists all jobs with enhanced metadata for frontend display"""
    try:
        conn = get_conn()
        cursor = conn.execute("""
            SELECT job_id, state, progress, created_at, updated_at
        ...
        jobs = []
        for row in cursor.fetchall():
            # Determine display status based on state and progress
            display_status = row[1]  # Use actual state from database
            if row[1] in ["processing", "running"] and row[2] > 0:
                ...
                display_status = "queued" if row[2] == 0 else "processing"
            ...
            jobs.append({
                ...
                "created_at": row[3],
                "updated_at": row[4],
                "display_name": f"Quick Create Job {row[0][:8]}",
                "type": "quick_create_full_universe",
                "status_message": f"Status: {display_status} - Progress: {row[2]}%"
            })
        conn.close()
        return {"jobs": jobs}
    except Exception as e:
...
async def root():
    return {
        "message": "Gen Scene Studio Backend - Worker Fixed + Hub Jobs Endpoint!",
        "version": "1.0.0-HUB-FIXED",
