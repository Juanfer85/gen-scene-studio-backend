...
log = setup_logging()
app = FastAPI(title="Gen Scene Studio Backend", version="1.0.0-ROBUST")
...
def safe_timestamp_to_iso(timestamp):
    """Safely convert timestamp to ISO format, handling None and invalid values"""
    if timestamp is None:
        return None
    try:
        # Convert to seconds if it's a large number (assuming milliseconds)
        if timestamp > 9999999999:  # If it's likely in milliseconds
    ...
    try:
        ...
        for row in cursor.fetchall():
            jobs.append({
                ...
                "state": row[1],
                "progress": row[2],
                "created_at": safe_timestamp_to_iso(row[3]),
                "updated_at": safe_timestamp_to_iso(row[4]) if len(row) > 4 else safe_timestamp_to_iso(row[3])
            })
        conn.close()
        return {"jobs": jobs}
    except Exception as e:
...
def list_jobs_hub(_k=Depends(require_api_key)):
    """Hub Jobs endpoint - Enhanced version with robust date formatting for frontend"""
    try:
        conn = get_conn()
        cursor = conn.execute("""
            SELECT job_id, state, progress, created_at, updated_at
        ...
        for row in cursor.fetchall():
            ...
            created_at_iso = safe_timestamp_to_iso(row[3])
            updated_at_iso = safe_timestamp_to_iso(row[4]) if len(row) > 4 else created_at_iso
...
async def root():
    return {
        "message": "Gen Scene Studio Backend - Worker Fixed + Hub Jobs + Robust Date Handling!",
        "version": "1.0.0-ROBUST",
        "worker_method": "BackgroundTasks (No ASGI lifespan issues)",
        "hub_jobs_endpoint": "/api/jobs-hub",
        "date_fix": "Robust timestamp conversion with error handling",
        "docs": "/docs",
        "health": "/health",
