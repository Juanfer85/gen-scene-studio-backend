...
log = setup_logging()
app = FastAPI(title="Gen Scene Studio Backend", version="1.0.0-HUB-ADDED")
...
def list_jobs_hub(_k=Depends(require_api_key)):
    """Hub Jobs endpoint - Enhanced version for frontend display"""
    try:
        conn = get_conn()
        cursor = conn.execute("""
            SELECT job_id, state, progress, created_at, updated_at
...
async def root():
    return {
        "message": "Gen Scene Studio Backend - Worker Fixed + Hub Jobs Endpoint Added!",
        "version": "1.0.0-HUB-ADDED",
        "worker_method": "BackgroundTasks (No ASGI lifespan issues)",
        "hub_jobs_endpoint": "/api/jobs-hub",
        "standard_jobs_endpoint": "/api/jobs",
        "docs": "/docs",
        "health": "/health",
