...
async def worker():
    ...
    while True:
        try:
            ...
            # Update progress with proper state names
...
def ensure_worker_started():
    """
    Ensure worker is started - called from both startup event and first job creation
    """
    if not hasattr(app.state, 'worker_task') or app.state.worker_task.done():
        try:
            ...
            app.state.worker_task = loop.create_task(worker())
            log.info("✅ Worker task started from ensure_worker_started()")
            return True

        except Exception as e:
            log.error(f"❌ Failed to start worker from ensure_worker_started(): {e}")
            import traceback
            traceback.print_exc()
            return False

    log.info(f"Worker status check: already_exists={hasattr(app.state, 'worker_task')}")
    return False
...
# Startup event - inicia el worker automáticamente al arrancar
@app.on_event("startup")
async def startup_event():
    """
    FastAPI startup event handler
    """
    log.info("🚀 FastAPI startup event fired")
...
async def shutdown_event():
    ...
    """
    log.info("🛑 FastAPI shutdown event fired")

    # Cancel worker task if it exists and is not done
    if hasattr(app.state, 'worker_task'):
        worker_task = app.state.worker_task
        if not worker_task.done():
            log.info("Cancelling worker task due to shutdown")
            worker_task.cancel()
            try:
                await worker_task
            except asyncio.CancelledError:
                log.info("Worker task cancelled successfully")
...
async def quick_create_full_universe(request: QuickCreateRequest, _api_key=Depends(require_api_key)):
    """
    Create a new quick-create-full-universe job and ensure worker is running
    """
    try:
        # Generate job IDs
        ...
        # ENSURE WORKER IS STARTED - this is the key addition
        ensure_worker_started()
...
async def delete_job(job_id: str, _k=Depends(require_api_key)):
    """
    Delete a job from database and queue
    """
    try:
        conn = get_conn()
        ...
        log.exception("Failed to delete job")
        raise HTTPException(status_code=500, detail=str(e))
...
# NOTA: Este archivo ya no incluye asyncio.create_task(worker()) al final
# El worker se inicia exclusivamente a través de los eventos de FastAPI