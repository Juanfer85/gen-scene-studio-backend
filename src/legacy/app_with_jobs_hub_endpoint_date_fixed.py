...
log = setup_logging()
app = FastAPI(title="Gen Scene Studio Backend", version="1.0.0-DATE-FIXED")
...
def list_jobs_hub(_k=Depends(require_api_key)):
    """Hub Jobs endpoint - Enhanced version with proper date formatting for frontend"""
    try:
        conn = get_conn()
        cursor = conn.execute("""
            SELECT job_id, state, progress, created_at, updated_at
        ...
        jobs = []
        for row in cursor.fetchall():
            # Convert Unix timestamp to proper JavaScript Date format
            created_at_timestamp = row[3]
            updated_at_timestamp = row[4] if len(row) > 4 else created_at_timestamp

            # Convert Unix timestamp to datetime and then to ISO format
            created_at_iso = datetime.fromtimestamp(created_at_timestamp).isoformat() if created_at_timestamp else None
            updated_at_iso = datetime.fromtimestamp(updated_at_timestamp).isoformat() if updated_at_timestamp else None
...
def get_job_status(job_id: str, _k=Depends(require_api_key)):
    try:
        ...
        created_at_iso = datetime.fromtimestamp(row[3]).isoformat() if row[3] else None
        updated_at_iso = datetime.fromtimestamp(row[4]).isoformat() if len(row) > 4 else created_at_iso
...
async def root():
    return {
        "message": "Gen Scene Studio Backend - Worker Fixed + Hub Jobs + Date Fixed!",
        "version": "1.0.0-DATE-FIXED",
        "worker_method": "BackgroundTasks (No ASGI lifespan issues)",
        "hub_jobs_endpoint": "/api/jobs-hub",
        "date_fix": "ISO format for frontend compatibility",
        "docs": "/docs",
        "health": "/health",
