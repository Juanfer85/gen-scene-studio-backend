...
log = setup_logging()
app = FastAPI(title="Gen Scene Studio Backend", version="0.4.0")
    ...
    while worker_running:
        try:
            ...
            cursor = conn.execute("""
                ...
                WHERE state = 'quick_create_full_universe' AND progress = 0
...
async def _startup():
    ...
    try:
        ...
        log.info("Worker task started successfully")
    except Exception as e:
        log.error(f"Failed to start worker task: {e}")
        # Fallback: try to start worker manually after delay
        async def delayed_start():
            await asyncio.sleep(5)
            try:
                asyncio.create_task(worker())
                log.info("Worker task started successfully (delayed)")
            except Exception as e2:
                log.error(f"Failed to start delayed worker: {e2}")
