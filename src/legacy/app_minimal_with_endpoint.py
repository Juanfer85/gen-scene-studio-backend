...
def _bin_ok(name: str) -> bool:
    """Chequea si un binario existe y es ejecutable."""
    try:
...
def require_api_key(x_api_key: Optional[str] = Header(default=None, alias="X-API-Key")):
    """Dependencia para validar API key con protección contra timing attacks."""
    expected = settings.BACKEND_API_KEY.strip()
    if not expected:
        return  # sin clave → modo dev
...
async def get_database_stats(_k=Depends(require_api_key)):
    '''Get database performance statistics'''
    try:
        from core.db_optimized import get_connection_stats
        ...
        log.exception('Failed to get database stats')
        raise HTTPException(status_code=500, detail=str(e))

@app.post('/api/database/optimize')
async def optimize_database_endpoint(_k=Depends(require_api_key)):
    '''Run database optimization and maintenance'''
    try:
        from core.db_optimized import optimize_database
        ...
        log.exception('Database optimization failed')
        raise HTTPException(status_code=500, detail=str(e))

@app.get('/api/database/performance')
async def get_database_performance(_k=Depends(require_api_key)):
    ...
    try:
        ...
        # Get performance-related PRAGMAs
        performance_metrics = {}

        pragmas_to_check = [
            'journal_mode', 'synchronous', 'cache_size', 'mmap_size',
            'temp_store', 'page_size', 'locking_mode', 'busy_timeout',
            'wal_autocheckpoint', 'auto_vacuum'
        ]
        ...
        for pragma in pragmas_to_check:
            try:
                ...
                performance_metrics[pragma] = result[0] if result else 'unknown'
            except Exception:
                performance_metrics[pragma] = 'error'
        ...
        return {
            'status': 'success',
            'timestamp': time.time(),
            'pragmas': performance_metrics,
            'tables': table_stats
        }

    except Exception as e:
        log.exception('Failed to get database performance')
        raise HTTPException(status_code=500, detail=str(e))