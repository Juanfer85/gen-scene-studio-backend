...
# Startup event - inicia el worker automáticamente
@app.on_event("startup")
async def start_worker():
    log.info("Starting worker task with FastAPI startup event...")
    try:
        ...
        log.info("Worker task started successfully with FastAPI startup event")
...
# Shutdown event - detiene el worker correctamente
@app.on_event("shutdown")
async def stop_worker():
    log.info("Stopping worker task...")
    try:
        # Get the worker task from app state
        ...
        if task and not task.done():
            ...
            log.info("No worker task to stop")
...
async def delete_job(job_id: str, _k=Depends(require_api_key)):
    """Delete a job from the database and queue"""
    try:
        conn = get_conn()
        ...
        log.info(f"Job {job_id} deleted successfully")
...
# NOTA: No hay código asyncio.create_task(worker()) al final del archivo
# El worker se inicia exclusivamente a través del evento @app.on_event("startup")